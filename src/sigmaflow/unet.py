...
https://github.com/Benjamin-Walker/diffrax-diffusion-models/blob/master/models/UNet.py
"""
...
class UNet(eqx.Module):
    ...
    def __call__(self, y, *, t=0, key=None):
        t = self.time_pos_emb(t)
        t = self.mlp(t)
        y = rearrange(y, "w h c -> c w h")
        h = self.first_conv(y)
        hs = [h]
        ...
        for layer in self.final_conv_layers:
            h = layer(h)
        return rearrange(h, "c w h -> w h c")
